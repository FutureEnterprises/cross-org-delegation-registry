...
- **evidence:** `tests/evidence-graph.test.js` and `tests/admissibility-profiles.test.js` exercise the AEG and admissibility-replay carrier in-repo; the adjacent `evidence-record.v1.json` (5) and `provenance.exec.v1.json` (7) suites run under `node conformance/run.mjs`. `evidence_type: local_harness` — the AEG and admissibility mechanism is covered by EP's own in-repo JavaScript tests, not by a cross-language runner; the evidence-record and provenance suites that ARE cross-language are stated on the C-007 row rather than claimed here. Vectors, counts, and paths are reproducible at EP repo commit `bdfb4a82f7c7` (the same discipline the registry applies to `evaluated_against` draft revisions).
...
- **evidence:** `provenance.exec.v1.json` (7), `evidence-record.v1.json` (5), `witness.v1.json` (6), and `timestamp-proof.v1.json` (13), all run under `node conformance/run.mjs`. `evidence_type: cross_language_consistency` — EP's JavaScript, Python, and Go verifiers agree on these suites; this is one team in one repository, a consistency check, not independent reimplementation. (An externally authored from-spec Rust verifier, public source, is reported on the list to agree on the 162-vector set it was run against (the tri-language suite is now 163); that external agreement is auditable in its source and is not claimed here as the row's `evidence_type`.) Vectors, counts, and paths are reproducible at EP repo commit `bdfb4a82f7c7` (the same discipline the registry applies to `evaluated_against` draft revisions).
...
- **evidence:** `revocation.exec.v1.json` (6), `time-attestation.v1.json` (6), and `timestamp-proof.v1.json` (13), all run under `node conformance/run.mjs`. `evidence_type: cross_language_consistency` — EP's JavaScript, Python, and Go verifiers agree; one team, one repository, a consistency check, not independent reimplementation. Vectors, counts, and paths are reproducible at EP repo commit `bdfb4a82f7c7` (the same discipline the registry applies to `evaluated_against` draft revisions).
...
- **evidence:** `boundary.v1.json` cases `raw_claim_pass_through` and `same_party_evidence_presented_as_independent` (a four-vector suite run under `node conformance/run.mjs`, JavaScript / Python / Go agree); `examples/binding/human-authorization-binding-vector.mjs` case B3 (verified-but-never-accepted). `same_party_evidence_presented_as_independent` is cited as consistent with the C-011 accepted-result constraint (reproduction under the issuer's own key is not independent verification), not as isolating the independence property on its own: it refuses on two converging grounds, the signature does not cover the embedded `independent_verification` bytes, and even if it did a same-key self-attestation never establishes independence, so a plain signature failure would refuse it regardless. `evidence_type: cross_language_consistency` — the boundary suite is checked by EP's JS/Python/Go verifiers, one team and one repository, a consistency check; the binding example is an in-repo JavaScript worked case supporting it. Vectors, counts, and paths are reproducible at EP repo commit `bdfb4a82f7c7` (the same discipline the registry applies to `evaluated_against` draft revisions).
